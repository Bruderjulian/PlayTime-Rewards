...
public class Text {
  ...
  public static void error(
    ...
    severe(
      ...
      PlayerPlayTime.instance.getDescription().getName() +
      "."
    );
    ...
    if (
      disable &&
      Bukkit.getPluginManager().isPluginEnabled(PlayerPlayTime.instance)
    ) {
      Bukkit.getPluginManager().disablePlugin(PlayerPlayTime.instance);
    }
  }
  ...
  public static void reload() {
    prefix =
      PlayerPlayTime.instance
        .getConfig()
        .getString("prefix", "&7[&6Playtime&7]") +
      "&r";
    hasPlaceholderAPI =
      Bukkit.getPluginManager().isPluginEnabled("PlaceholderAPI");

    try {
      final String bukkitVersion = PlayerPlayTime.instance
        .getServer()
        .getBukkitVersion();
    ...
    final File messageFile = new File(
      PlayerPlayTime.instance.getDataFolder().getAbsolutePath(),
      "messages.yml"
    );
    if (!messageFile.exists()) {
      PlayerPlayTime.instance.saveResource("messages.yml", false);
    }
    ...
    messageCache.clear();
    for (final String key : messagesConfig.getKeys(true)) {
